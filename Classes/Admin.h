...
public:
    Admin(string first, string last, string nCode, string bDay, bool pos) : User(first, last, nCode, bDay, pos)
    {
        loadUsers();
    }
    ...
    bool deleteUser(int option, string code)
    {
        ...
        if (option == 0)
        {
            ...
            strcpy(fileNameChar, fileName.c_str()); // Make File name a char array
        ...
        else
        {
            ...
            deleteFromUsersFile(deleteFile);
            users.clear();
            loadUsers(); // Update Users List
        }
        return true;
    }
    ...
    void groupDelete()
    {
        ...
        string code;
        while (getline(codes, code, ' '))
            deleteUser(1, code); // Delete each code in mode 1
    }
    ...
    stringstream askInfo(int option)
    {
        ...
        { // Delete User
            ...
            cin >> code;
            return stringstream(code);
        }
        break;
        case 2:
        {
            cout << "Enter National Codes, Separate by space: ";
            string input;
            cin.ignore();
            getline(cin, input);
